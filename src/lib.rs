...
impl Quantization for MedianCut {
    fn build(&mut self) {
        ...
        while buckets_count < self.shades_count {
            ...
            for b in 0..buckets_count {
                ...
                bucket.iter().for_each(|p| {
                    if p.xyz[0] < x.0 { x.0 = p.xyz[0]; }
                    if p.xyz[0] > x.1 { x.1 = p.xyz[0]; }
    
                    if p.xyz[1] < y.0 { y.0 = p.xyz[1]; }
                    if p.xyz[1] > y.1 { y.1 = p.xyz[1]; }
    
                    if p.xyz[2] < z.0 { z.0 = p.xyz[2]; }
                    if p.xyz[2] > z.1 { z.1 = p.xyz[2]; }
                });
                ...
                bucket.sort_by(|p1, p2| {
                    p1.xyz[highest_space].cmp(&p2.xyz[highest_space])
                });
            }
        ...
        for b in 0..buckets_count {
            ...
            bucket.into_iter().for_each(|point| point.xyz = max_xyz);
        }
    ...
    fn get_palette(&mut self) -> Vec<String> {
        self.palette.iter().map(|[r, g, b]|
            format!("#{:02X?}{:02X?}{:02X?}", r, g, b)
        ).collect()
    }

    fn to_buffer(&mut self) -> Vec<u8> {
        self.space.iter().flat_map(|point| {
            point.xyz
        })
        .collect()
    }
